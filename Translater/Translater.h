...
namespace MsgPassingCommunication
{
	...
	public:
		Translater();
		void setSendFilePath(String^ sendFilePath);
	...
	void Translater::setSendFilePath(String^ path) {
		if (pComm != nullptr) pComm->setSendFilePath(Sutils::MtoNstr(path));
	}

	void Translater::setSaveFilePath(String^ path) {
		if (pComm != nullptr) pComm->setSaveFilePath(Sutils::MtoNstr(path));
	}
  //----< initialize endpoint >----------------------------------------
